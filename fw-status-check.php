...
@$link = mysql_connect( '192.168.7.121', 'fw3648605250', 'y5Lq9ybCkDumtgQUWsAcjEjdBBy0Rm' );
if ( ! $link ) {
	die( sprintf( "Could not connect to the MySQL server: %s\n", mysql_error() ) );
}
echo "Connected successfully to the MySQL server\n";

@$db_selected = mysql_select_db( db0101629280, $link );
if ( ! $db_selected ) {
	die ( sprintf( "Could not select the database '%s': %s\n", 'db0101629280',  mysql_error() ) );
}
echo "Database selected successfully\n";

echo "\nChecking tables for errors:\n";
$tables = mysql_query( "SHOW TABLES LIKE 'wp_xajunljlcx_%'" );
